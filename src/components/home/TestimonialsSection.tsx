...
function Testimonial({
  ...
  return <Card className="h-full border border-gray-100 hover:shadow-md transition-shadow">
      <CardContent className="p-6 flex flex-col h-full rounded-2xl">
        <div className="flex items-center gap-2 mb-4">
          {[...Array(5)].map((_, i) => <StarIcon key={i} size={16} className={i < rating ? "fill-yellow-400 text-yellow-400" : "text-gray-200"} />)}
        ...
        <div className="flex items-center mt-auto pt-4 border-t border-gray-100">
          <div className="w-10 h-10 rounded-full bg-gray-200 flex items-center justify-center text-gray-500 font-medium mr-3">
            {imgSrc ? <img src={imgSrc} alt={name} className="w-full h-full rounded-full object-cover" /> : name.charAt(0)}
          </div>
          <div>
...
export function TestimonialsSection() {
  ...
  const currentTestimonials = testimonials.slice(activeIndex * testimonialsPerPage, (activeIndex + 1) * testimonialsPerPage);
  const goToPrevious = () => {
  ...
  return <section id="testimonials" className="section-padding">
      <div className="container mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 rounded-2xl overflow-hidden">
          {currentTestimonials.map((testimonial, index) => <Testimonial key={index} {...testimonial} />)}
        </div>
        
        {totalPages > 1 && <div className="flex justify-center items-center gap-4 mt-10">
            <Button variant="outline" size="icon" onClick={goToPrevious} className="h-10 w-10 rounded-full">
              ←
            </Button>
            ...
            <Button variant="outline" size="icon" onClick={goToNext} className="h-10 w-10 rounded-full">
              →
            </Button>
          </div>}
      </div>
