        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-center">
          <div className="order-2 md:order-1">
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              <Button onClick={openWhatsApp} size="lg" className="bg-dental-500 hover:bg-dental-600 text-white font-medium px-8 rounded-xl">
                Book a Consultation
              </Button>
              <Button onClick={() => document.getElementById('services')?.scrollIntoView({
              behavior: 'smooth'
            })} variant="outline" size="lg" className="border-dental-200 hover:bg-dental-50 text-dental-800 font-medium rounded-xl">
                Explore Services
              </Button>
            </div>
          </div>
          <div className="order-1 md:order-2 overflow-hidden rounded-2xl shadow-2xl">
            <img src="https://images.unsplash.com/photo-1606811971618-4486d14f3f99?q=80&w=1000&auto=format&fit=crop" alt="Beautiful smile transformation" className="w-full h-auto object-cover" />
          </div>
        </div>
      </div>
    </section>;
